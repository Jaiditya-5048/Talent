...
function getIncludes(includeVenue = true) {
  ...
  includes.push(
    ...
    {
      model: TicketType,
      attributes: ['ticket_type_id', 'price', 'total_seats', 'available_seats', 'createdAt', 'updatedAt']
    }
  );
...
export const createInstanceRepo = async (instanceData) => {
  const instance = await EventInstance.create(instanceData);

  const data = {
    instance_id: instance.instance_id,
    artist_id: instanceData.artist_id,
  };
